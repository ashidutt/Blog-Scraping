...
from GS.Python_Google_Search import gsearch_url_form_class


# Options
GS_LINK_JSON_FILE       = r'C:\data\temp\output'
RESULT_FILE             = r'c:\data\temp\search_string_data.txt'
MYFILE                  =r'c:\data\temp\Google_Scraping_Results.txt'
...
class GoogleSearch(Spider):
    ...
    def parse(self, response):
        ...
        if self.setting_data['type_of_parse'] == 'general':
            ...
            # New code added on 5th Nov 2015. If not working then delete it from here
            count=0
            with open(MYFILE,'a+') as j:
                #j.write('\n')
                #j.write('UniqueID')
                #j.write('\n')
                j.write(str(count))
                ...
                j.write('keyword')
                j.write('|')
                #j.write(search_words)
                j.write(title)
                j.write('|')
                j.write(response.url)
                j.write('|')
                j.write(str(contents))
                j.write('\n')
                '''
                for line in myfile:
                    line = line.lower()
